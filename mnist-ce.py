...
train_zero_l=[]                 #targets are '0' and '8'
train_zero=[]
train_eight_l=[]
train_eight=[]
...
train_zero = np.asfarray(train_zero) / 255.0        #normalization
...
def fi(x):                          #derivative f
    return 1/((1-x + 0.01))

def psi(x):                         #derivative y
    return -1/((x))
...
def mult(z,B):                      # B*ReLU multiplication
    k=np.zeros(shape=(300,))
    for i in range(300):
        if(z[i]>0):
            k[i]=B[i]
...
def cost(x1,x2):
    return (-1)*np.log(1-x1 + 0.01)-np.log(x2 + 0.01)       #add 0.01 so as not to divide by 0
